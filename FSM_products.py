...
async def load_category_product(message: types.Message, state: FSMContext):
    ...
    await FSM_store.next()
    await message.answer(text='Укажите артикул(айди) товара:')
...
async def submit(message: types.Message, state: FSMContext):
    await message.answer(text="Напишите 'Да' если хотите сохранить данные в бд")
    if message.text == "Да":
        ...
        await message.answer(text="Несохранено")
        await state.finish()


def product_fsm(dp: Dispatcher):
